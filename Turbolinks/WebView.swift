...
class DebugWebView: WebView {
	...
	func setupViews() {
		let label = UILabel()
		label.frame = CGRect(x: 10, y: 400, width: 300, height: 60)
	...
	func debug(scriptMessage: ScriptMessage) {
		debugLabel.text = String(describing: scriptMessage.name)
	}
	
	func debug(function: String, visit: Visit) {
		debugLabel.text = String(format: "%@: %@", function, visit.debugDescription)
	}
	...
	override func layoutSubviews() {
		...
		debugLabel.frame = CGRect(x: 10, y: bounds.size.height - 80, width: 300, height: 60)
	}
}
...
extension WebView: WKScriptMessageHandler {
	func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
		...
		if let debugWebView = self as? DebugWebView {
			debugWebView.debug(scriptMessage: message)
		}
